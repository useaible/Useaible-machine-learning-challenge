...
def main():
    try:
        ...
        channel.queue_bind(exchange='amq.topic',
                        queue=queue_name,
                        routing_key="tensorflow.train.maze")
...
def train_tensorflow(data, channel):    
    ...
    train_route_key = 'tensorflow.actions.maze.' + user_token
    play_route_key = 'tensorflow.play_maze.' + user_token
...
def training_test(cntr):
    nn = neural_network()
    tf.reset_default_graph()
    game = mazeSimulator(maze_height, maze_width,grid,starting_position['X'], starting_position['Y'],goal_postion['X'], goal_postion['Y'], perfect_game_moves)
    # game = LanderSimulator()
    nn.env = game   
    nn.init_network(game)
    print "Thread %r started training at %r." % (cntr, datetime.datetime.now().time())
    nn.train(1000)
    nn.play()
    print "Thread %r started training at %r." % (cntr, datetime.datetime.now().time())
